...
FSccSyncImplTool::FSccSyncImplTool(ISCCModule& InSCCModule)
	: SCCModule(InSCCModule)
{
}

FString FSccSyncImplTool::GetName() const
{
	return TEXT("scc_sync");
}

FString FSccSyncImplTool::GetDescription() const
{
	return TEXT("Sync files to latest revision from source control");
}

TSharedPtr<FJsonObject> FSccSyncImplTool::GetInputSchema() const
{
...
TSharedPtr<FJsonObject> FSccSyncImplTool::Execute(const TSharedPtr<FJsonObject>& Arguments)
{
	...
	FSCCOperationResult OpResult = SCCModule.Sync(Files);
