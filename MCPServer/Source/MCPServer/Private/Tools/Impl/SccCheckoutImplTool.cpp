...
FSccCheckoutImplTool::FSccCheckoutImplTool(ISCCModule& InSCCModule)
	: SCCModule(InSCCModule)
{
}

FString FSccCheckoutImplTool::GetName() const
{
	return TEXT("scc_checkout");
}

FString FSccCheckoutImplTool::GetDescription() const
{
	return TEXT("Check out files from source control");
}

TSharedPtr<FJsonObject> FSccCheckoutImplTool::GetInputSchema() const
{
...
TSharedPtr<FJsonObject> FSccCheckoutImplTool::Execute(const TSharedPtr<FJsonObject>& Arguments)
{
	...
	FSCCOperationResult OpResult = SCCModule.Checkout(Files);
